...
public class BankingServiceAdapter implements BankingPort {
    ...
    @Override
    public boolean requestFirmbanking(String FromBankName, String FromBankAccountNumber, String ToBankName, String ToBankAccountNumber,int moneyAmount) {
        String buildUrl = String.join("/", this.bankingServiceEndpoint,"banking", "firmbanking","request");
        try {
            ObjectMapper mapper = new ObjectMapper();
            String requestBody = mapper.writeValueAsString(new RequestFirmBankingRequestBody(FromBankName,FromBankAccountNumber,ToBankName,ToBankAccountNumber,moneyAmount));
            CompletableFuture<HttpResponse<String>> responseFuture = bankingServiceHttpClient.sendPostRequest(buildUrl, requestBody);
            return responseFuture.thenApply(response -> {
                try {
                    // 응답 본문을 MoneyChangingResponseBody 객체로 변환
                    RequestFirmBankingResponseBody responseBody = mapper.readValue(response.body(), RequestFirmBankingResponseBody.class);
                    // 성공 여부 판단 (예: balance가 0보다 크면 성공)
                    return responseBody.getFirmbankingStatus() == 1; // 성공 여부 반환
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            }).join(); // CompletableFuture의 결과를 동기적으로 대기
